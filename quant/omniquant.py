...
def omniquant(
    ...
    if "llama" in args.net.lower():
        ...
        raise ValueError(
            "Only support for opt/llama/Llama-2/falcon/mixtral now")
    ...
    # take output of quantization model as input
    ...
    for i in range(len(layers)):
                    ...
                    for key in pairs.keys():
                        if key in name:
                            ...
                            weight = module.weight.abs().max(
                                dim=0)[0].clamp(min=1e-5)
                            scale = (act.pow(args.alpha) /
                                     weight.pow(1-args.alpha)).clamp(min=1e-5)
                            if use_shift and not is_llama:
                                shift = act_shifts[f"{layer_name_prefix}.{i}.{name}"].to(
                                    device=dev, dtype=dtype)
                            else:
                                shift = torch.zeros_like(scale)
                            qlayer.register_parameter(
                                f"{pairs[key]}_smooth_shift", torch.nn.Parameter(shift))
                            qlayer.register_parameter(
                                f"{pairs[key]}_smooth_scale", torch.nn.Parameter(scale))
            ...
            for epochs in range(args.epochs):
                ...
                for j in range(args.nsamples//args.batch_size):
                    ...
                    with traincast():
                        ...
                        quant_out = qlayer(
                            quant_inps[index:index+args.batch_size,], attention_mask=attention_mask_batch, position_ids=position_ids)[0]
                        loss = loss_func(
                            fp_inps[index:index+args.batch_size,], quant_out)
                        if args.aug_loss:
                            loss += loss_func(fp_inps_2[index:index +
                                              args.batch_size,], quant_out)
                    ...
                    optimizer.zero_grad()
                    norm = loss_scaler(
                        loss, optimizer, parameters=get_omni_parameters(qlayer, use_shift)).cpu()
        ...
        if args.epochs > 0:
            ...
            torch.save(omni_parameters, os.path.join(
                args.output_dir, f"omni_parameters.pth"))
        else:
            register_scales_and_zeros(qlayer)
            layers[i] = qlayer.to("cpu")
        if args.real_quant:
            # only support weight-only quantization
