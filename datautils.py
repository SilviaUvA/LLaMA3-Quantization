...
def get_pile(nsamples, seed, seqlen, model):
    ...
    traindata = load_dataset(
        "json", data_files='/cpfs01/user/chenmengzhao/prompt_quantization/val.jsonl.zst', split="train", trust_remote_code=True)

    tokenizer = AutoTokenizer.from_pretrained(model, use_fast=False)
    trainenc = tokenizer("\n\n".join(
        traindata['text'][:1000]), return_tensors='pt')
...
def get_c4(nsamples, seed, seqlen, model):
    ...
    for _ in range(nsamples):
        while True:
            ...
            # TODO: will break with randint if == sqlen
